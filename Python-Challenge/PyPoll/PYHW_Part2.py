...
#Macro to tally up a candidates percentage total using their index within the
...
#Each candidates percentage total is put into a list where the Max_Index macro is run to find the index
...
with open('PythonHW_Part2_Election_Results_SDB.csv', 'w') as datafile:
    ...
    #CandidateTotal macro (rounded to 3 decimal places), and uses Candidate.count(UniqueCandidate[]) to display the
    ...
    print(f"And The Win is: {UniqueCandidate[WinnerIndex]}!", file = datafile)
...
print("--------------------------------")
#The winner is determined using the WinnerIndex number within the UniqueCandidate list.
print(f"And The Win is: {UniqueCandidate[WinnerIndex]}!")
